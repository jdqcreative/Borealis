...
LINMATH_H_FUNC void mat4x4_perspective(mat4x4 elements, float y_fov, float aspect, float n, float f)
{
	/* NOTE: Degrees are an unhandy unit to work with.
	 * linmath.h uses radians for everything! */
	float const a = 1.f / tanf(y_fov / 2.f);

	elements[0][0] = a / aspect;
	elements[0][1] = 0.f;
	elements[0][2] = 0.f;
	elements[0][3] = 0.f;
	...
	elements[2][0] = 0.f;
	elements[2][1] = 0.f;
	elements[2][2] = -((f + n) / (f - n));
	elements[2][3] = -1.f;

	elements[3][0] = 0.f;
	elements[3][1] = 0.f;
	elements[3][2] = -((2.f * f * n) / (f - n));
	elements[3][3] = 0.f;
}
LINMATH_H_FUNC void mat4x4_look_at(mat4x4 elements, vec3 const eye, vec3 const center, vec3 const up)
{
	...
	mat4x4_translate_in_place(elements, -eye[0], -eye[1], -eye[2]);
}
